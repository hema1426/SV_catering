...
class MultipleSelectSpinnerAdapter(
    ...
    fun customDropdownView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        if (convertView == null) {
            returnView =
                LayoutInflater.from(myContext).inflate(
                    R.layout.spinner_checkbox_dropdown, null
                )
        ...
        viewHolder.rlItem.isEnabled = dataList[position].isEnabled
        viewHolder.isSelected.isEnabled = dataList[position].isEnabled
